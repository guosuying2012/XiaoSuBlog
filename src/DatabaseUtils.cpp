...
//查询所有分类
void DatabaseUtils::queryAllSorts(cppdb::session& sql, bool bIsAdmin, sorts& vecRes)
{
    cppdb::result resRecords;
    sort record;
    resRecords.clear();
    ...
    try
    {
        resRecords = sql << "SELECT sort_id, sort_parent_id, sort_name, sort_rank FROM yengsu_sorts where sort_isadmin = ?" << bIsAdmin;
        while (resRecords.next())
        {
